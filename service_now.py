...
def calcular_prioridade(df, hoje=None):
    """
    Calcula, para cada chamado, colunas derivadas de priorização:
      ...
      _prioridade: rank numérico (menor = mais urgente) — vencidos primeiro (mais
                   atrasado primeiro), depois não vencidos por % do ANS já decorrido
    """
    if hoje is None:
    ...
    # rank: vencidos (0..) ordenados pelo maior atraso primeiro; depois não vencidos
    # abertos ordenados por % do ANS já decorrido (mais perto de vencer primeiro)
    rank_vencido = (-d["_dias_atraso"].fillna(0)).where(d["_vencido"], other=0)
    rank_aberto_ok = (-perc.fillna(0)).where(d["_aberto"] & ~d["_vencido"], other=0)
    grupo = d["_vencido"].map({True: 0, False: 1}) + (~d["_aberto"]).map({True: 1, False: 0})
    d["_prioridade"] = list(zip(grupo, rank_vencido.where(d["_vencido"], rank_aberto_ok)))
    return d
...
def kpis_sn(df):
    ...
    return dict(total=total, abertos=abertos, vencidos=vencidos,
                concluidos=concluidos, pct_no_prazo=pct_no_prazo)


COLUNAS_RESUMO = [
    SN_TAREFA, SN_CATEGORIA, SN_STATUS, SN_ABERTO, SN_PRAZO,
    "_dias_atraso", "_dias_restantes", SN_SOLICITANTE, SN_LOTACAO,
    SN_RESPONSAVEL, SN_DESCRICAO,
]

COLUNAS_RESUMO_NOMES = [
    "Tarefa", "Categoria", "Status", "Aberto em", "Prazo (ANS)",
